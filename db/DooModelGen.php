...
class DooModelGen {
  ...
  public static function genSqlite($extends = '', $createBase = FALSE, $addmaps = FALSE, $filenameModelPrefix = '', $baseSuffix = 'Base', $chmod = NULL, $path = NULL) {
    ...
    // get table names
    $tables = Doo::db()
      ->query("SELECT name FROM sqlite_master where (type='table' or type='view') and name<>'sqlite_sequence'")
  ...
  public static function genPgSQL($path = NULL) {
    ...
    $smt = Doo::db()
      ->query("SELECT table_name as name FROM INFORMATION_SCHEMA.tables WHERE table_schema = '$dbSchema'");
    $tables = $smt->fetchAll();
    foreach ($tables as $tbl) {
      ...
      $smt4 = Doo::db()->query("SELECT t.relname, a.attname, a.attnum
                ...
                LEFT JOIN pg_attribute a
                    ...
                    WHERE t.relname = '$tblname'
                       ");
//     $indkey = isset($fields3[0]['indkey'])?$fields3[0]['indkey']:null;
//      AND a.attnum = '{$fields3[0]['indkey']}'
      $fields4 = $smt4->fetchAll();
      $pkey = isset($fields4[0]['attname']) ? $fields4[0]['attname'] : "";
      //Prepare model class
      $classname = '';
      $temptbl = $tblname;
      for ($i = 0; $i < strlen($temptbl); $i++) {
